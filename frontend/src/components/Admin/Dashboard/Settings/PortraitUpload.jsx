...
const PortraitUpload = () => {
  ...
  const handleUploadAvatar = (avatar) => {
    setIsDragging(false)
      ...
      if (avatar.size > 2000000) {
        handleUploadError({data: {detail: 'File size must not exceed 2MB(megabytes)'}, status: null})
        return;
      }
      if (uploadError !== null) {
        return;
  ...
  const dragDropIcon = () => {
    if (isDragging && !isLoading) {
      return (<Icon height="68px" width="68px" color="#9b9393" as={RiAddFill}></Icon>);
    } else if (!isDragging && isLoading) {
      return (<Spinner loading={isLoading} size={68} />);
    }
    return (<Icon height="68px" width="68px" color="#9b9393" as={FiUpload}></Icon>);
  }

  return (
    <Box mt={10} mb="auto" mr={3}>
      <Text mb={2} color="dark.secondary">Change Avatar:</Text>
      {uploadError && <Text fontWeight="bold" fontSize="12px" color="validationError.primary" mb={2}>{ uploadError }</Text>}
      <Box
      position="relative"
...
// when changing avatar url need to update local storage
// when